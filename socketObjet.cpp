...
int SocketObjet::getSocket(int id) { return (id < 0 ? socket_ : socketfd_[id]); }

vector<int> SocketObjet::getSockets() { return socketfd_; }

SocketObjet::SocketObjet(int newBacklog) : backlog(newBacklog), socket(socket(AF_INET, SOCK_STREAM, 0)) {
    if(socket_ == -1) {
        perror("Socket : ");
        exit(1);
    }
...
SocketObjet::~SocketObjet() noexcept {}
...
void SocketObjet::fermer(int sockfd) {
 ...
 void SocketObjet::envoyerGroup(char* c, const vector<int>& destinataires) {
